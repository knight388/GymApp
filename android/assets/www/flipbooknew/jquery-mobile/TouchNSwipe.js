...
(function(window, $)
{
	...
	function ImageSlider(slider, _slides, varsObj)
	{
		...
		function _addSlide(imgUrl, caption)
		{
			if($.type(imgUrl) === "string")
			{
				var slide = {};
				slide.url = imgUrl;
				...
				slide.image = $("<img style='position:absolute; max-width:none;'>").on("load",  { index:_slides.length }, onImageLoad).error( { index:_slides.length }, onImageError );
				TweenMax.set(slide.image, { autoAlpha:0 } );
				_slides.push(slide);
				
				if(_slides.length > 2)
				{
					_vars.loop = originalLoop;		
				}
			}
			else
			{
				console.log("Added an unknown type.");	
			}
		...
		function onPreloaderImageLoad(e)
		{
			...
			TweenMax.set([prevPreloader, curPreloader, nextPreloader], { position:"absolute", maxWidth:"none", left:"50%", top:"50%", x:"-50%", y:"-50%"} );
		}
...
(function(window, $)
{
	...
	function PhysicsScroller(scroller, _thumbs, varsObj)
	{
		...
		function _addThumb(thumbUrl)
		{
			if($.type(thumbUrl) === "string")
			{
				var thumb = {},
					borderColor = (_thumbIndex == _thumbs.length) ? _vars.borderColor : _vars.defaultBorderColor;
				...
				thumb.loaded = false;
				thumb.resized = false;
				thumb.image = $("<img style='position:absolute; max-width:none;'>").on("load",  { index:_thumbs.length }, onImageLoad);
				thumb.div = $("<div style='position:absolute; overflow:hidden; width:" + _vars.thumbWidth + "px; height:" + _vars.thumbHeight + "px; left:" + ((_vars.thumbWidth + _vars.space)* _thumbs.length) + "px; border:" + _vars.borderThickness + "px solid " + borderColor + ";' >");
				
				if(tempPreloaderImage !== null)
				{
					thumb.preloaderImage = $("<img style='position:absolute; max-width:none; display:block;'>").on("load",  { index:_thumbs.length }, onPreloaderImageLoad);
					thumb.div.append(thumb.preloaderImage);
				}
				
				hiddenDiv.append(thumb.image);
				_thumbsHolder.append(thumb.div);
				TweenMax.set(thumb.image, { autoAlpha:0 } );
			...
			{
				console.log("Thumb url is not string"); 	
			}
		}
		...
		function onPreloaderImageLoad(e)
		{
			...
			if(!_thumbs[i].loaded)
			{
				TweenMax.set(_thumbs[i].preloaderImage, { position:"absolute", left:"50%", top:"50%", x:"-50%", y:"-50%" } );
		...
		function _index(i)
		{
			if(i !== undefined && !isNaN(i))
			{
				...
				{
					_thumbIndex = i;
					if(oldThumbIndex >= 0 && oldThumbIndex < _thumbs.length)
					{
...
(function(window, $)
{
	...
	TouchNSwipe.addSlide = function(idOrIndex, imageUrl, thumbUrl, caption)
	{
		var tnsObj = TouchNSwipe.getObj(idOrIndex);
		
		if(tnsObj != null)
		{
			var slider = tnsObj.slider,
				thumbScroller = tnsObj.thumbScroller;
			
			if(slider != null)
			{
				slider.addSlide(imageUrl, caption);
			}	
			
			if(thumbScroller != null)
			{
				thumbScroller.addThumb(thumbUrl);
			}
		}
